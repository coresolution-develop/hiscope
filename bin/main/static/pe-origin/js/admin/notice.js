(() => {
  const API = {
    list:   '/admin/notices',          // GET : 목록(JSON)
    create: '/admin/notices',          // POST: 생성(JSON)
    update: id => `/admin/notices/${id}`, // PUT : 수정
    remove: id => `/admin/notices/${id}`  // DELETE
  };
  ...
  async function loadList(){
    ...
    try{
      ...
      if(!res.ok){
        ...
        console.error('GET /admin/notices failed', res.status, text);
